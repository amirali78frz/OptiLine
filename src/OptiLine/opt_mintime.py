...
from OptiLine.utils import calc_splines, calc_spline_lengths, nonreg_sampling, calc_head_curv_num, H_f
...
def opt_mintime(reftrack: np.ndarray,
    ...
    if pars["optim_opts"]["step_non_reg"] > 0:
        reftrack, discr_points = nonreg_sampling(track=reftrack,
        ...
        coeffs_x, coeffs_y, a_interp, normvectors = calc_splines(path=refpath_cl)
    ...
    # spline lengths
    spline_lengths_refline = calc_spline_lengths(coeffs_x=coeffs_x,
                                                                         coeffs_y=coeffs_y)

    # calculate heading and curvature (numerically)
    kappa_refline =  calc_head_curv_num(path=reftrack[:, :2],
    ...
    steps = list(range(discr_points_cl.size))
    ...
    print(n_min,n_max,w_tr_left_interp(0),pars["optim_opts"]["width_opt"])
    ...
    for k in range(N):
        ...
        g.append(((f_x_flk / (mue * f_z_flk)) ** 2 + (f_y_flk / (mue * f_z_flk)) ** 2))
        g.append(((f_x_frk / (mue * f_z_frk)) ** 2 + (f_y_frk / (mue * f_z_frk)) ** 2))
        g.append(((f_x_rlk / (mue * f_z_rlk)) ** 2 + (f_y_rlk / (mue * f_z_rlk)) ** 2))
        g.append(((f_x_rrk / (mue * f_z_rrk)) ** 2 + (f_y_rrk / (mue * f_z_rrk)) ** 2))
        lbg.append([0.0] * 4)
        ubg.append([1.0] * 4)
    ...
    diff_matrix = np.eye(N) - np.diag(np.ones(N - 1), 1)
    ...
    # for i, l in enumerate(lbw):
    #     if True:  
    #         print(f"lbw[{i}] has length {len(l)} and type {type(l)}: {l}")
    lbw = np.concatenate([np.array([float(x) for x in item]) for item in lbw])
    ubw = np.concatenate([np.array([float(x) for x in item]) for item in ubw])
    ...
    atot_opt = np.sqrt(ax_opt**2 + ay_opt**2)
    ...
    raceline_reopt = reftrack[:, :2] + np.expand_dims(-x_opt[:-1, 3], 1) * normvectors

    if plot_ == True:
        _, _, _, normvec_norm = calc_splines(path=np.vstack((reftrack[:, 0:2], reftrack[0, 0:2])),use_dist_scaling=True)
        ...
        plt.plot(bound1[:, 0], bound1[:, 1], 'k', label=' Outer Track')
        plt.plot(bound2[:, 0], bound2[:, 1], 'k', label=' Inner Track')
        plt.plot(raceline_reopt[:,0], raceline_reopt[:,1],'r.-' , label='Opt_ZO')
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.legend()
        plt.show()
        ...
        plt.subplot(1,2,1)
        plt.plot(s_opt, x_opt[:, 0],'b' , label='v_opttime')
        plt.ylabel('v_profile')
        plt.xlabel('distance')
        plt.legend()
        plt.grid(True)
        plt.subplot(1,2,2)
        plt.plot(s_opt, ax_opt,'b' , label='a_x')
        ...
        plt.show()
        # plt.subplot(1,3,3)
        # plt.plot(s_opt, t_opt,'b' , label='t_opttime')
        # plt.ylabel('time')
        # plt.xlabel('distance')
        # plt.grid(True)
        # plt.show()

    return -x_opt[:-1, 3], x_opt[:-1, 0],raceline_reopt, reftrack, a_interp, normvectors, s_opt[:-1], t_opt[-1]
